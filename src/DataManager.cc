  ...
  for(Int_t i=0;i<fNumSensitiveVols;i++)
    {
      ...
      G4cout<<"Name of output tree branch:  "<<Name.Data()<<G4endl;
...
//only fill event tree if any core has energy  > 0
void DataManager::FillEventTree() {
  Double_t detected_energy = 0.0;
  for (Int_t i = 0; i < fNumCores; ++i) {
    detected_energy += fEnergyDep[fCoreNamesArray[i]];
  }

  // if (detected_energy > 0.0) fEventTree->Fill();
  fEventTree->Fill();
