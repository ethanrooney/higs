  ...
  if(evnt->GetNumberOfPrimaryVertex()==1 && evnt->GetPrimaryVertex(0)->GetNumberOfParticle()==1)
    {
      ...
      // reverse the energy adjustment for Compton scattering so that the
      // energy distibution of the beam itself is saved
      //T = T / (1 + (T/(1875.6)) * (1-cos(theta)) );// for lD2 Compton scattering
      //PrimaryParticleKineticEnergy = PrimaryParticleKineticEnergy / (1-PrimaryParticleKineticEnergy/1875.6*(1-cos(PrimaryParticleTheta)));
  ...
  else
    {
      ...
      G4Exception("EventAction::BeginOfEventAction", "007", FatalException,
		  "INCORRECT GENERATION OF PRIMARIES IN PRIMARY GENERATOR ACTION!!**");
    }
